  ...
  return (
    <>
      ...
      >
       {/* LEFT FADE */}
<div
  className="pointer-events-none absolute left-0 top-0 h-full w-40 lg:w-56 xl:w-120 z-999 hidden md:block"
  style={{
    background:
...
{/* RIGHT FADE */}
<div
  className="pointer-events-none absolute right-0 top-0 h-full w-40 lg:w-56 xl:w-120 z-999 hidden md:block"
  style={{
    background:
...
/>
            ...
            >
              {[...roadmapData, ...roadmapData, ...roadmapData].map(
                (item, index) => (
                  ...
                  >
                    <div
                      className="text-[45px] font-bold leading-none mb-3 text-transparent"
                    ...
                    <h3 className="text-lg font-semibold text-black mb-3 font-poppins">
                      {item.title}
                    </h3>
      ...
      {/* ─── Mobile FIXED ───────────────────────────────────────── */}
      <section className="md:hidden relative w-full bg-[#F5F5F5] py-12 overflow-x-hidden">
          ...
          <div className="overflow-x-auto scrollbar-hide snap-x snap-mandatory">
            <div className="flex gap-4 pb-4">
              {roadmapData.map((item, index) => (
                <div
                  key={index}
                  className="flex-shrink-0 w-[85vw] bg-gradient-to-br from-[#d1d0d065] to-[#C0C0C0] rounded-[20px] snap-start min-h-[400px] flex flex-col justify-around"
                >
                  <div
                    className="text-[50px] font-bold text-transparent leading-none px-4 pt-4"
                    style={{ WebkitTextStroke: "1px #F5C518" }}
                  >
                    {item.number}
                  </div>

                  <div className="h-20 flex items-center justify-center p-25">
                    <img
                      src={item.icon}
                      alt={item.title}
                      className="object-contain max-w-full"
                  ...
                  <div className="px-5 pb-6">
                    <h3 className="text-md font-semibold text-black leading-tight mb-2 font-poppins">
                      {item.title}
                    </h3>
                    <p className="text-[13px] text-black leading-relaxed font-poppins">
                      {item.description}
                    </p>
                  </div>
                </div>
