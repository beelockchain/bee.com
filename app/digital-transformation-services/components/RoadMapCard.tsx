...
export default function RoadmapCarousel() {
  ...
  useEffect(() => {
    ...
    // ✅ Changed from 768px → 1024px so tablet uses mobile scroll view
    mm.add("(min-width: 1024px)", () => {
      const totalCards = roadmapData.length;
      const cardWidth = 270;
      const gap = 32;

      const moveDistance = (cardWidth + gap) * (totalCards - 1);
  ...
  return (
    <>
      ...
      <section
        ref={sectionRef}
        // ✅ Changed from hidden md:flex → hidden lg:flex
        className="hidden lg:flex flex-col justify-center relative w-full h-dvh bg-[#F5F5F5]"
      >
        {/* LEFT FADE */}
        <div
          className="pointer-events-none absolute left-0 top-0 h-full w-40 lg:w-56 xl:w-60 z-999"
          style={{
            background:
        ...
        {/* RIGHT FADE */}
        <div
          className="pointer-events-none absolute right-0 top-0 h-full w-40 lg:w-56 xl:w-60 z-999"
          style={{
            background:
      ...
      <section
        // ✅ Removed md:hidden — now shows on mobile AND tablet, hides at lg+
        className="lg:hidden relative w-full bg-[#F5F5F5] py-12 overflow-x-hidden"
      >
        <div className="px-4 max-w-full overflow-x-hidden">
          <div className="text-center mb-12">
            <h2 className="text-xl md:text-2xl font-semibold leading-tight text-black">
              From Strategy to Execution:{" "}
              <span className="shine-text relative text-[#f5c518]">
          ...
          <div className="overflow-x-auto scrollbar-hide snap-x snap-mandatory">
            <div className="flex gap-4 pb-4">
              {roadmapData.map((item, index) => (
                <div
                  key={index}
                  // ✅ Added md:w-[42vw] for a comfortable 2-card-peek layout on tablets
                  className="shrink-0 w-[85vw] sm:w-[55vw] md:w-[42vw] bg-linear-to-br from-[#d1d0d065] to-[#C0C0C0] rounded-[20px] snap-start min-h-[400px] flex flex-col justify-around"
                >
                  <div
