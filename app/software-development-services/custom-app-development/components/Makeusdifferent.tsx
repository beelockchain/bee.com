    ...
    <div className="w-full bg-white py-8 px-6 md:px-12">
      ...
      <div className="max-w-5xl mx-auto text-center mb-10">
        <h2 className=" text-[19px] sm:text-[19px] md:text-[21px] leading-tight lg:text-[28px] xl:text-[36px] text-black font-bold  mb-4 ">
          <span data-text="Beelockchain:" className="shine-text text-[#F5B800] relative mx-0">Beelockchain:</span> What Makes Us
          Different
          <br /> From The Rest
        </h2>
        <p className="text-black text-[14px] md:text-[12px] lg:text-[14px] xl:text-[16px] text-black leading-relaxed font-normal  mb-6 max-w-2xl mx-auto">
      ...
      <div ref={containerRef} className="max-w-6xl mx-auto flex flex-col lg:flex-row items-center lg:items-start gap-8 lg:gap-10 h-auto mt-6 lg:mt-0">
        ...
        <div className="w-full lg:w-1/2 flex flex-col gap-4 order-none lg:order-1">
          {cards.map((card) => (
            ...
            >
              <h3 className="text-left text-[14px] sm:text-[15px] lg:text-lg font-bold text-black relative z-10 pr-2 lg:pr-0">
                {card.title}
              </h3>
              <p className="text-left text-black text-[11.5px] sm:text-[13px] lg:text-sm xl:text-sm font-normal leading-relaxed relative z-10 pb-4 lg:pb-0 pr-8 lg:pr-0">
                {card.description}
              </p>
              <span className="absolute bottom-1 right-2 lg:static lg:self-end text-[48px] lg:text-2xl font-extrabold text-[#b8b8b8] lg:text-black leading-none pointer-events-none lg:pointer-events-auto z-0 lg:mt-2">
                {card.number}
              </span>
            </div>
          ))}
