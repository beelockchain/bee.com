...
const Appdevindustryweserve = () => {
  ...
  return (
    <section className="w-full bg-white py-10 overflow-hidden">
      {/* MOBILE TITLE */}
      <h2 className="px-10 md:px-0 block md:hidden text-[19px] sm:text-[19px] md:text-[21px] lg:text-[28px] xl:text-[36px] text-black leading-tight font-bold mb-8 text-center">
        Types Of
        <span
      ...
      <div className="max-w-[1200px] mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-6 md:gap-16 items-start">
        ...
        <div className="md:pt-2 lg:pt-0 lg:mt-4 text-left">
          ...
          <h2 className="hidden md:block text-[19px] sm:text-[19px] md:text-[21px] lg:text-[28px] xl:text-[36px] font-bold text-black mb-4  leading-tight text-center">
            Types Of <span className="text-[#F5A623]">Mobile Apps</span> We
            Build Multiple Use Cases
          </h2>

          <p className="max-w-xl mb-8 lg:mb-12 text-[14px] md:text-[12px] lg:text-[14px] xl:text-[16px]  text-black leading-relaxed font-semibold ">
          ...
          <div className="grid grid-cols-3 gap-x-10 gap-y-5 mb-10 lg:mb-14">
            {industries.map((item, i) => (
              <span
                key={i}
                className="text-[13px] sm:text-[13px] md:text-[12px] lg:text-[14px] xl:text-[18px] text-black underline underline-offset-2 cursor-pointer hover:text-yellow-500 transition-colors"
              >
                {item}
              </span>
            ))}
